...
void doRefCheck(TString file, bool makepsfiles=false, bool writeRef=false) {
  ...
  while ( infile->good()) {
	...
	if ( datums.size() > 0 ) {
	  ...
	  TH1F *hTemp=new TH1F(tTitle,tTitle,datums.size()-2,refLow,refHigh);
  ...
  if ( datums.size() > 0 ) {
    ...
    tTitle+=references.size();
    TH1F *hTemp=new TH1F(tTitle,tTitle,datums.size()-2,refLow,refHigh);
    for ( unsigned int i=0; i<datums.size(); i++) hTemp->SetBinContent(i,datums[i]);
    references[refTitle]=hTemp;
  }
  ...
  for ( unsigned int i=0; i<names.size(); i++) {
	...
	std::cout << " reference has different binning! " << h1->GetNbinsX() << " " << h2->GetNbinsX() << std::endl;
      }
      else{
	int nDiffs=0;
	for ( int i=0; i<h1->GetNbinsX(); i++ ) 
	  if ( h1->GetBinContent(i) != h2->GetBinContent(i) ) nDiffs++;
	if ( nDiffs > 0 )
	  std::cout << " has " << nDiffs << " bins that were different from reference" << std::endl;
	else
    ...
    if ( makepsfiles ) {
      TString psName=file;
      psName+="_"; psName+=i; psName+=".ps";
      can->Print(psName);
    }
  }

  if ( outf != 0 ) outf->close();
